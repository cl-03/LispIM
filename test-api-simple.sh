...
API_BASE="http://localhost:8443/api/v1"
PASS=0
FAIL=0

echo "========================================"
echo "LispIM API v1 简化测试"
echo "========================================"
echo ""
...
check_server() {
    echo "检查服务器状态..."
    curl -s --connect-timeout 2 "${API_BASE%/*}/healthz" > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "❌ 服务器未运行在 http://localhost:8443"
        echo ""
        echo "请启动服务器："
        echo "  ./scripts/start.sh start"
        echo ""
        echo "或者在开发模式下运行："
        echo "  sbcl --load lispim-core/lispim-core.asd"
        echo "  (ql:quickload :lispim-core)"
        echo "  (lispim-core:start-server)"
        exit 1
    fi
    echo "✓ 服务器正在运行"
    echo ""
