#!/usr/bin/env python3
"""Check paren balance for api-register-fcm-token-handler function"""

with open(r'D:\Claude\LispIM\lispim-core\src\gateway.lisp', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find the function end (next defun or comment at column 0)
start_line = 2189  # 0-indexed
...
if depth != 0:
    print(f"Need to add {-depth} closing parens" if depth > 0 else f"Need to remove {depth} closing parens" if depth < 0 else "")
